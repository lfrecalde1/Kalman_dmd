...
%% Initial Conditions
v_estimate(:, 1) = [X_r(:, 1)];
y_estimate(:, 1) = Cd*v_estimate(:, 1);
%% Eps valu
eps_n = 0.5;
eps_d = 0.001;%% Simulation

%% Initial States Kalman
x_estimate =  v_estimate(:, 1);
P_k = zeros(4, 4, length(t)+1);
P_k(:, :, 1) = 1*eye(4);
...
for k= 1:length(t)
    ...
    %% System Values
    v_estimate(:, k+1) = Ad*v_estimate(:, k) + Bd*U_r(:, k) + eps_d*randn(4, 1);
    y_estimate(:, k+1) = Cd*v_estimate(:, k+1)+ eps_n*randn(4, 1);
    
    %% Kalman Filter
    x_es = Ad*x_estimate(:, k) + Bd* U_r(:,k);
    P = Ad*P_k(:, :, k)*Ad' + Q;
    
    %% Estimation
    Kk = P*Cd'*inv(Cd*P*Cd' + R);
    x_estimate(:, k+1) = x_es + Kk*(y_estimate(:, k+1)-Cd*x_es);
    P_k(:, :, k+1) = (eye(4) - Kk*Cd)*P;
end
